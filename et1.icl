...
flen :: String -> Int
flen str = length [ x \\ x <-: str ]

//Start = flen Jack.firstName
 
instance toInt Person
where 
     toInt p = p.age + p.height + flen p.firstName
...
where 
     isEven p = isEven (p.height + p.age)
...
//isPrime :: String -> Bool 
isPrime arr = length [len rem x \\ x <- [1..len] ] == 2
where 
     len = length (filter ( (==) 'i' ) [ x \\ x <-: arr ])

//Start = isPrime "Delhi"

continentsPrimeI :: [Continent] -> [String]
continentsPrimeI list = [ x.contName \\ x <- list | (or[ isPrime y.capital \\ y <-: x.countries ]) ]
...
shouldWePlay list g = sum[1 \\ p <- list | p.skillLevel >= g.skillLevel] >= 3
...
full_words num 
| num < 0 = "minus" +++ "-" +++ full_words(abs num)
| num < 10 = numberList.[num]
= numberList.[num rem 10] +++ "-" +++ full_words (num/10)
...
is2 :: Int [Int] -> Int
is2 num list = length [ x \\ x <- list | x == num ]
 
//Start = is2 3 [1,2,3,45,6,7,3] 
 
f7 :: {Int} -> Bool
f7 arr = and [ (is2 x y) >= 2 \\ x <- y ]
where 
    y = [ a \\ a <-: arr ]
