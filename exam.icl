...
-- < DEEPAK KUMAR UPADHAYAY , DD79DY > for the end-term programming assignment of
...
| [ x.id \\ x <- list | x.level == BSc && avg (x.grades) == A ] <> [] = hd [ x.id \\ x <- list | x.level == BSc && avg (x.grades) == A ]
= abort "Error"
where 
     A = maxList [ avg (x.grades) \\ x <- list | x.level == BSc ]
...
toChar :: String -> [Char]
toChar list = [ x \\ x <-: list ] 

countAux :: [Char] Char -> Int
countAux list c = sum [ 1 \\ x <- list | x == c ]

//Start = countAux (toChar "thequickbrownfoxjumpsoverthelazydog") 'e'

//count :: String -> {(Char, Int)}
count str = removeDup[ (x,countAux s x) \\ x <- s | countAux s x > 1 ]   
where 
     s = toChar str
...
getBTHeight (BTNode l r)
| l <> BTLeaf = 1 + (getBTHeight l) 
= 1 + (getBTHeight r) 
...
Insert (Pointer a Nil) value = Pointer a (Pointer value Nil)
Insert (Pointer a b) value =  (Pointer a (Insert b value))
...
firstAndMiddle Leaf = []
firstAndMiddle (Node (LastName a) b c) = firstAndMiddle b ++ firstAndMiddle c
firstAndMiddle (Node (FirstName a) b c) = [a] ++ firstAndMiddle b ++ firstAndMiddle c
firstAndMiddle (Node (MiddleName a) b c) = [a] ++ firstAndMiddle b ++ firstAndMiddle c
