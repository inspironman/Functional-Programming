...
remMax :: {Int} -> {Int}
remMax arr = { i \\ i <-: arr | i <> maxList [i \\ i <-: arr] }
...
getLength :: [[Int]] -> {Int}
getLength list = {sum [x!!y\\x<-list] \\ y<-[0..(len list-1)]}


sumArrays :: [{Int}] -> {Int}
sumArrays list = getLength [ [y \\y <-:x] \\ x <- list]
...
change x
| x.status == Good = { x & gpa = x.gpa + 0.1 }
= x

bestStudent :: [Student] -> Student
bestStudent list =  maxList [ change x \\ x <- list ] 
*/
