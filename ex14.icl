...
/**1
  Write a function, that takes a list of functions, and a list of tuples (Int, Int) where the first Int indicates
  which function to use and the second Int acts as a parameter and returns a list of the results.
...
Router [] tuplist = []
Router funlist [] = []
Router funlist tuplist = [ ((funlist) !! ((fst x)-1)) (snd x) \\ x <- tuplist ]
...
/**2
  Write a function that takes a list of integers and returns a list of result integers 
  based on how many integers were in the parameter list.
...
checkEven :: [Int] -> Bool
checkEven [] = False
checkEven list = checkEvenAux ( sort list)
...
/**6
   ...
   ('0'..'9'), the second part contains the rest. */

TwoLists :: [Char] -> ([Char], [Char])
TwoLists list = ( [c \\ c <- list | c >= '0' && c <= '9'] , [n \\ n <- list | n >= 'a' && n <= 'z']) 
...
unzip3 (a,b,c) = sort [(a),(b),(c)]

//Start = map unzip3 [(3,4,5),(4,5,6),(4,5,3),(6,8,10),(10,5,8),(-3,4,5)]

f8Aux :: [(Int,Int,Int)] -> [Int]
f8Aux list = flatten (removeDup [ unzip3 x \\ x <- list ])

//Start = f8Aux [(1,1,1),(5,4,3),(3,4,5),(0,0,0)] //[(5,4,3)]

 
f8 :: [(Int,Int,Int)] -> [(Int,Int,Int)]
f8 list =  [ (x,y,z) \\ x <- (f8Aux list) , y <- (f8Aux list) , z <- (f8Aux list) | (x*x) + (y*y) == (z*z) ]
...
f9 :: [Int] -> Bool
f9 list = and [ x == (toInt(sqrt (toReal x)))^2 \\ x <- list ]
